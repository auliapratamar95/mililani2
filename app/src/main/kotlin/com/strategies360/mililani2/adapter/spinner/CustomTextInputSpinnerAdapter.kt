...
import kotlinx.android.synthetic.main.adapter_custom_spinner_textinput.view.*

class CustomTextInputSpinnerAdapter<DATA_TYPE>(context: Context) : EasierSpinnerAdapterCore<DATA_TYPE, CustomTextInputSpinnerAdapter<DATA_TYPE>.ViewHolder, CustomTextInputSpinnerAdapter<DATA_TYPE>.ViewHolder, CustomTextInputSpinnerAdapter<DATA_TYPE>.DropDownViewHolder>(context) {
    ...
    override fun onCreatePlaceholderViewHolder(parent: ViewGroup): ViewHolder {
        val view = parent.inflate(R.layout.adapter_custom_spinner_textinput)
    ...
    inner class ViewHolder(itemView: View) : EasierSpinnerViewHolder(itemView) {
        ...
        init {
            itemView.apply {
                txt_custom_input.setOnClickListener { easierSpinner.showSelection() }
            }
        }

        fun bindView(data: DATA_TYPE?, isPlaceholder: Boolean) {
            itemView.txt_custom_input.setTextSize(TypedValue.COMPLEX_UNIT_PX, context.resources.getDimension(R.dimen.fo_font_16))
            itemView.apply {
                txt_custom_input.setText(
                        if (isPlaceholder) {
