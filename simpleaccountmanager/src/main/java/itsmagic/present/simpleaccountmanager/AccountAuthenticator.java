...
public class AccountAuthenticator extends AbstractAccountAuthenticator {
    ...
    public Bundle getAuthToken(AccountAuthenticatorResponse response, Account account, String authTokenType, Bundle options) throws NetworkErrorException {
        // Check the calling package before doing anything
        boolean isValidPackage = false;
        PackageManager packageManager = mContext.getPackageManager();
        String callerPackageName = options.getString("androidPackageName");

        // Caller app must be signed with the same key to get the auth token
        int signatureResult = packageManager.checkSignatures(BuildConfig.APPLICATION_ID, callerPackageName);
        if (signatureResult >= PackageManager.SIGNATURE_MATCH) isValidPackage = true;

        // Check if the caller requested an authToken type we don't support
        if (!authTokenType.equals(AccountHelper.getAccountType())) isValidPackage = false;
        if (isValidPackage) {
