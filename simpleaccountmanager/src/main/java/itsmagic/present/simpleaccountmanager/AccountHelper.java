...
public class AccountHelper {
    ...
    public static void init(
        ...
        init(accountType, accountManagerActivity, new Pair<String, Long>(null, 0L));
    }
    ...
    public static void init(
            @NonNull final String accountType,
            @NonNull final Class accountManagerActivity,
            @NonNull final Pair<String, Long> ... syncAdapters) {

        mAccountType = accountType;
        mAccountManagerActivity = accountManagerActivity;
        if (syncAdapters.length == 1 && syncAdapters[0].first == null) mSyncAdapters = null;
    ...
    public Account getAccount(
        ...
        final String accountName = getSelectedAccountName();
    ...
    private void addAccountExplicitly(
        ...
        if (!isLoggedIn(accountName)) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                final Map<String, Integer> accountVisibility = new HashMap<>();
                accountVisibility.put(mAccountType, AccountManager.VISIBILITY_VISIBLE);
                accountManager.addAccountExplicitly(account, null, null, accountVisibility);
            } else {
    ...
    public String getSelectedAccountName() {
        String accountName = AccountPreferenceHelper.getInstance(mContext).loadSelectedAccount();
        if (isLoggedIn() && accountName == null) {
            // Select the very first account as selection if user is actually still logged in
            // This may happen when the application's data is cleared
            setSelectedAccountNameAsFirstAccount();
            accountName = AccountPreferenceHelper.getInstance(mContext).loadSelectedAccount();
        }
